...
# Get AI response using OpenRouter API with Claude model
def get_ai_response(prompt, conversation_history):
    ...
    # Set up headers with OpenRouter API key and optional headers for leaderboard
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://ai-career-counsellor.app",  # Optional: Your site URL for rankings
        "X-Title": "AI Career Counsellor"  # Optional: Your app name for rankings
    }
    
    # Prepare messages in the same format as before
    messages = conversation_history + [{"role": "user", "content": prompt}]
    
    # Update the model to use Claude
    data = {
        "model": "anthropic/claude-3-haiku",  # Using Claude 3 Haiku - fast and cost-effective
        "messages": messages
    }
    
    # The endpoint remains the same for OpenRouter
